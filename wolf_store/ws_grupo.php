<?php
    ...
    if (!moduloPermissao('Gerir Grupo', $conn)) {
...
<body>
    <script>
        ...
        function editorRegistro(id,tabela) {
            fetch('ws_grupo_editor.php?id='+encodeURIComponent(id)+'&tabela='+encodeURIComponent(tabela))
            .then(response => response.text())
            .then(html => {
        ...
        function editRegistro(id,tabela) {
                ...
                if(confirm("Registrar alteração?")) {
                    ...
                    .then(data => {
                        if (data === "ok") {
                            window.location.href = "ws_grupo.php"
                        } else {
        ...
        function createRegistro() {
            ...
            fetch('scripts/ws_add_grupo.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
    ...
    <div class="conteudo">
        <ul class="breadcrumb">
            <li><a href="#" onclick="voltarPagina()"><span class="icon-start"></span>Grupos</a></li>
        </ul>
